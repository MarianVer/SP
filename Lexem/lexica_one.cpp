...
// ВАЖЛИВО: Я закоментував USE_DFA, щоб працювали наші текстові Regex правила нижче,
// а не старі жорстко прописані таблиці переходів.
...
// 1. ЗМІНИ В TOKENS_RE (Розпізнавання токенів)
// Видалено символи: +, -, ==, !=, !!, &&, || (бо у вашому варіанті це слова ADD, EQ, NOT...)
// Додано символи: >, < (бо вони є у вашому варіанті)
// Залишено: ->, дужки, коми, крапка з комою.
#define TOKENS_RE         "->|\\(|\\)|\\{|\\}|\\[|\\]|,|;|\\>|\\<|[_0-9A-Za-z]+|[^ \t\r\f\v\n]"

// 2. ЗМІНИ В KEYWORDS_RE (Список ключових слів)
// Додані ваші оператори як слова: ADD, SUB, MUL, DIV, MOD, EQ, NE, NOT, AND, OR
// Змінено регістр службових слів: PROGRAM, VAR, BEGIN, END...
// Додано цикл: FOR, DOWNTO, DO
// Додано тип: LONG, INT
#define KEYWORDS_RE       "->|\\(|\\)|\\{|\\}|\\[|\\]|,|;|\\>|\\<|PROGRAM|VAR|BEGIN|END|SCAN|PRINT|IF|ELSE|FOR|DOWNTO|DO|ADD|SUB|MUL|DIV|MOD|EQ|NE|NOT|AND|OR|LONG|INT"

// 3. ЗМІНИ В IDENTIFIERS_RE (Формат змінних)
// Варіант 2: Нижня риска і дві великі літери (наприклад _AB)
#define IDENTIFIERS_RE    "_[A-Z][A-Z]"

// 4. ЗМІНИ В UNSIGNEDVALUES_RE (Числа)
// Залишаємо без змін, цілі числа
...
#define DEFAULT_INPUT_FILE "../base_test_programs_2025/file2.k03"
...
int main(int argc, char* argv[]) {
	...
	// ЗМІНА КОМЕНТАРІВ: замість #*...*# використовуємо ??...?? (Варіант 2)
